...
- 처리 계획/진행:
  - 학습 일지·선택 기록·Issue 처리 기록 작성
  - 실제 4-table 도메인만 쓰는 외부 평가 요청서 작성
  - learning/eval 브랜치를 목적에 맞춰 최신화
  - 병합 후 전체 refs 이메일 rewrite, old→new SHA 매핑 보관, fresh clone 검증
